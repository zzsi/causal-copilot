...
class Analysis(object):
    ...
    def estimate_effect_MetaLearners(self, outcome, treatment, T0, T1, X_col, W_col, query):
        print("Before Fix - Unique Treatment Values:", self.global_state.user_data.processed_data[treatment].unique())
        # 🔹 Fix Treatment Variable (Ensure it's Binary)
        self.global_state.user_data.processed_data[treatment] = (
            self.global_state.user_data.processed_data[treatment] > 0
        ).astype(int)

        print("After Fix - Unique Treatment Values:", self.global_state.user_data.processed_data[treatment].unique())

        # 🔹 Ensure `X_col` and `W_col` are properly selected
        if X_col is None:
