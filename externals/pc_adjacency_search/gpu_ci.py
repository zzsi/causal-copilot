...
def gpu_row_pvals_ln(x_id, candidates, candidates_y, lvl, cols_map):
    ...
    for sList in splitted:
        if candidates_y.size == 0:
            break
        restricted_permutation_all = gpucmiknn.rperm_multi_all(obs.astype(np.float32), sList.astype(np.int64), globals.permutations, x_id.item())
        k = int(globals.observations_shape[0] * 0.2) if globals.k_cmi == 'adaptive' else globals.k_cmi ### use 0.2 as factor from Runge paper
        sOfX, pvalOfX, candidates_y = gpucmiknn.pval_ln_row(obs.astype(np.float32), x_id.item(), restricted_permutation_all, k+1, globals.k_perm+1, globals.permutations, lvl, sList.astype(np.int64), globals.alpha, np.array(candidates_y), len(candidates), isSplitted)
